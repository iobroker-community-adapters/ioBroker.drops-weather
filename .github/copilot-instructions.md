...
### Drops Weather Adapter Context

This adapter provides rain forecast data from the drops.live website. It's a weather adapter that uses web scraping to extract rain predictions and make them available to ioBroker.

**Key Features:**
- Fetches rain data from https://www.drops.live (previously drops.live)
- Provides 5-minute and 1-hour rain forecasts
- Uses Puppeteer for web scraping with stealth plugin to avoid detection
- Supports chart data for Material Design widgets 
- Handles ARM architecture (Raspberry Pi) with chromium-browser package
- Scheduled execution every 5 minutes

**Technical Specifications:**
- **Data Source**: drops.live website (external web scraping)
- **Primary Technology**: Puppeteer + Stealth Plugin for headless browser automation
- **Configuration**: City code-based (GPS coordinates no longer supported by website)
- **Data Points**: Rain intensity (mm/h), timestamps, chart data (JSON), boolean rain status
- **Browser Requirements**: Chromium/Chrome for Puppeteer, special handling for ARM architecture
- **Update Frequency**: Every 5 minutes via cron schedule

**Unique Requirements:**
- Browser path configuration for different platforms (automatic, built-in, external, chromium-browser)
- Timeout handling for page loads and network requests  
- JavaScript evaluation within Puppeteer for extracting RainGraph data from website scripts
- Temperature folder configuration for temporary files
- Language localization support (weekday names)
- Error handling for website structure changes and network issues
...
### Drops Weather Specific Standards

**Puppeteer Integration:**
- Always use `puppeteer-extra` with `puppeteer-extra-plugin-stealth` to avoid bot detection
- Implement proper browser lifecycle management (create, use, destroy)
- Handle browser timeouts appropriately (configurable via `browserTimeout` setting)
- Support multiple browser modes: automatic, built-in, external, chromium-browser
- For ARM architecture, ensure chromium-browser package availability

**Data Processing:**
- Parse JavaScript objects from website script tags containing RainGraph.create({})  
- Handle both 5-minute and 1-hour data intervals
- Convert timestamps to appropriate formats for ioBroker states
- Create chart-compatible JSON structures for Material Design widgets
- Implement proper error handling for missing or malformed data

**Configuration Management:**
- Use city codes instead of GPS coordinates (website limitation)
- Support browser path configuration for different deployment scenarios
- Handle language settings for weekday localization
- Manage temporary folder paths appropriately
...
- Ensure adapter stops cleanly on configuration or network errors

**Browser Resource Management:**
```javascript
// Always ensure proper browser cleanup
